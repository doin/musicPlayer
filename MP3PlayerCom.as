	...
	public class MP3PlayerCom extends Sprite
	{
		...
		public function onPlay($index:int=-1):void
		{
			...
			try {
				...
				//mp3Sound.addEventListener(ProgressEvent.PROGRESS, progress_Handler);
		...
		private function time_Handler(_evt:Event):void{
			...
			if(_curTime != curTime && _countTime != "NaN:NaN") {
				curTime = _curTime;
				ExternalInterface.call("S.MusicPlayer.progress", {index:curIndex, curtime:_curTime, counttime:_countTime, progress:_progress, load:_load});
			}
		}
		/**
		 *  音量设置
		...
		public function sendStatus($status:String=null):void
		{
			...
			ExternalInterface.call("S.MusicPlayer.status", _obj);
		}
		/**
		 *  发送错误事件 
		...
		public function sendError($msg:String="内容异常!"):void
		{
			...
			ExternalInterface.call("S.MusicPlayer.error", _obj);
		}
